...
def cdf(a_vals, figs, plot=False):
    likely_hood_x_y = polyfitter(x_vals, a_vals, figs, sigmas, plot)
    figs = likely_hood_x_y[3]
    try:
        plt.close(5)
        plt.close(6)
    except:
    ...
    for axi in [1,2]:
        ...
        # area_a[-1] is equivalent to max_a
        c_d_f = area_a/max_a
        ...
        if plot==True:        
            ...
            ax0.scatter(a_vals[axi-1], likely_hood_x_y[axi], marker='+')
            ax0.plot((a_best, a_best), ((np.max(likely_hood_x_y[axi]), 0)), 'g')
            ax0.set_ylim(bottom=0)
            ax0.set_ylabel(r'a$_'+str(axi-1)+'$ Likelihoods', fontsize=14)
            #ax0.set_xlabel(r'a$_'+str(axi-1)+'$ values')

            #CDF plot
            ax1.scatter(a_vals[axi-1], c_d_f, marker='+')
            ax1.plot((a_best, a_best), (( c_d_f.max(), 0)), 'g')
            ax1.errorbar(a_best, 0.5, xerr=[[uncertainties[0]], [uncertainties[1]]], fmt='^', color='red')
        ...
        '''
    return a_best, uncertainties, figs





def mwpolyfit(x_vals, a_values, figs, sigmas=1, plot=False):
    if type(sigmas) ==float or type(sigmas)==int: sigmas = np.ones(len(x_vals))*sigmas  
    ...
    #powers = len(a_values)
    # get data vector
    D_P_M_fig = mwpoly(x_vals, a_values, figs, sigmas, plot)
    D_vec = D_P_M_fig[0]
    ...
    B_mat_T = B_mat.T
    A_vec = inv( np.mat(B_mat)*np.mat(B_mat_T) )*np.mat(B_mat)*np.mat(D_vec).T  # A-->nx1 vector
    # matrix dot products
    ...
    if plot==True:
        A_vector = A_vec
        if Nby1==True: A_vector = A_vec.T
        A_vec_result= mwpoly(x_vals, np.array(A_vector)[0], figs, sigmas=1, plot=False)[2]
        print(A_vec_result)
        plt.figure(figs)
        figs+=1
        plt.clf()
        plt.scatter(x_vals,D_vec)
        plt.plot(x_vals, A_vec_result)

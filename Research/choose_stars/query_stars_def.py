...
def choose_primary(p_star_table, p_star_indice=0):        
    ...
    midnight_time = datetime.timedelta(0, longer_than_24hrs_and_Diff(following_sunrise - sunset)[1] /2.0 *3600.00) + sunset
    ...
    return observable, p_star_indice, time_l_visible, risetime, settime, diff_transit_meridian_and_midnight, trans_meridian_time, start_time
...
def find_comparison(primary_S_indice):
    ...
    while len(Comp_table) < 3:
        radius += 5
        Comp_table = customS.query_criteria('region(circle, '+p_star_table['ID'][primary_S_indice]+', '+str(radius)+'d)', 'maintypes=*', 'maintypes!=V*', 'Vmag < '+str(mags_V_p[primary_S_indice]), 'Vmag > '+str(mags_V_m[primary_S_indice])) # if so, then just search within a 10 degree radius rather than a 5 degree
    return Comp_table
...
for telnum in np.array([1, 2, 3, 4]):
    ...
    for ind in xrange(len(comparison_S_tables)):
        ...
        starttime = best_observables[ind][-1]
